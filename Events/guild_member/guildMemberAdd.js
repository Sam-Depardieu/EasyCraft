...
module.exports = {
    ...
    async execute(client, member) {
        ...
        const channel = client.channels.cache.get(guildStettings.welcomeChannelID);
        const role = client.roles.cache.get(guildStettings.nwMemberRoleID);
        if(!channel) return;
        console.log(`Nouveau membre (${member.id}) guildmemberadd`)
        ...
        const embed = new MessageEmbed()
            ...
            .setFooter({text: `N\'hésite pas à aller lire le réglement.`, iconURL: client.user.displaydisplayAvatarURL()})
